 ...
 *
 */
public class GiveAccessibilityPermission_Honor extends BaseAccessibilityJob  {
	   private static GiveAccessibilityPermission_Honor current;
	   ...
	   private static final String SWITCH_CLASS_NAME="android.widget.Switch";
	   ...
	   private String mAppName;
	    ...
	    private GiveAccessibilityPermission_Honor() {
	    	super(new String[]{SETTING_PACKAGENAME});
	    }
	    public static synchronized GiveAccessibilityPermission_Honor getGiveAccessibilityPermission_Honor() {
	        if(current == null) {
	            current = new GiveAccessibilityPermission_Honor();
	        }
	        return current;
	    }
	    @Override
		...
		*/
		public void clickServerButton(AccessibilityNodeInfo rootNode ){
			  ...
			  nodeInfo=AccessibilityHelper.findNodeInfosByClassName(rootNode, SWITCH_CLASS_NAME, -1, true);
			  if(nodeInfo==null)return;	
		    if(!bForward){
		    		AccessibilityHelper.performBack(service);
		    		return;
		...
		public void clickStopOnDialog(AccessibilityNodeInfo rootNode ){
			  //AccessibilityNodeInfo nodeInfo=AccessibilityHelper.findNodeInfosByText(rootNode, "开启服务将会：",0);
			  //if(nodeInfo==null)return;	
			  AccessibilityNodeInfo nodeInfo=AccessibilityHelper.findNodeInfosByText(rootNode, "是否停用",0);
			  if(nodeInfo==null)return;
			  nodeInfo=AccessibilityHelper.findNodeInfosByText(rootNode, "点击“确定”将会停用",0);
			  if(nodeInfo==null)return;
			  nodeInfo=AccessibilityHelper.findNodeInfosByTextAndClassName(rootNode, "确定",BUTTON_CLASS_NAME);
			  if(nodeInfo==null)return;
			  AccessibilityHelper.performClick(nodeInfo);//点击;
